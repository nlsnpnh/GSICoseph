...
import {
  useAnexos, useUploadAnexo, useDeleteAnexo, getAnexoSignedUrl, type ChamadoAnexo,
} from "@/data/api";
import { toast } from "@/hooks/use-toast";

export function AnexosSection({
  chamadoId, podeRemover = true,
}: {
  chamadoId: string;
  /** Remover anexo segue a regra de exclusão do chamado (admin/gestor). */
  podeRemover?: boolean;
}) {
  const { data: anexos = [], isLoading } = useAnexos(chamadoId);
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      try {
        await upload.mutateAsync({ chamadoId, file });
  ...
  const handleDownload = async (anexo: ChamadoAnexo) => {
    try {
  ...
  const handleDelete = async (anexo: ChamadoAnexo) => {
    try {
  ...
  return (
    <Card className="overflow-hidden border-border/80 shadow-sm">
      <FioAcento />
      <CardHeader className="flex flex-row items-center justify-between border-b border-border bg-muted/30 px-3 py-2">
